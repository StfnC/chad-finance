...
def create_trade(symbol, buy_price, quantity, portfolio, email, password):
    ...
    response = requests.post(f"{BASE_API_URL}/api/trade/", data = data, headers = my_headers)
    
    if(str(response.status_code) == '2'):
       print(response.json())
...
def test():
    ...
    for x in range(5):
        ...
        portfolio = x+2
        print("- email:"+email)
        create_trade(symbol, buyprice, quantity*x+2, portfolio,email, password)
